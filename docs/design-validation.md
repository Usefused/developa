...
- `make check` includes these gates alongside the existing backend/frontend checks. `make race` with `DEVELOPA_TEST_DATABASE_URL` also exercises the real Git/PostgreSQL/HTTP integration suite.
