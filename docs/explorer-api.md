...
Set `OLLAMA_ANALYSIS_MODEL` for background Features and `OLLAMA_ANSWER_MODEL` for interactive answers. Each falls back to `OLLAMA_MODEL`; they have independent execution gates. Models are operator configuration, never selected by repository content or request bodies.
...
`/features` accepts `q`, `limit`, and `offset` only; selection and pagination happen in SQL. Feature IDs are 64 lowercase hexadecimal characters and remain readable in the latest continuation. Each citation's name/path/span comes from indexed symbols rather than model-provided coordinates. The browser shows saved results and offers generation/continuation plus focused **Explain with AI** buttons on symbols/features. Free-form questions remain API-only. Browsing alone never generates an explanation.
