...
- When a workspace is added through a loopback Denverr origin, the selected absolute folder is also saved as that repository's editor root in localStorage. Remote origins do not assume the server checkout exists on the browser's machine; their editor mapping remains explicit.
