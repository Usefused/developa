...
export function EditorActions({item}) {
  ...
  return <><div className="detail-actions">{href ? <EditorLink path={item.path} position={position} className="primary-button"/> : <Button onClick={settings}>Set up editor ↗</Button>}<Button onClick={copy}>Copy location</Button></div><p role="status">{status}</p></>;
}
