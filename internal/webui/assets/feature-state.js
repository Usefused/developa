...
export async function loadFeaturePage(api, snapshot, offset, request, preferSaved = false) {
  let page = await api.features(snapshot.id,{limit:24,offset},request.signal);
  if (!request.current()) return null;
  if (preferSaved && !page.run && page.saved_snapshot) {
    snapshot = page.saved_snapshot;
    page = await api.features(snapshot.id,{limit:24,offset:0},request.signal);
  }
