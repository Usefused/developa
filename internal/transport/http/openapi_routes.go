...
func openAPIEndpoints() []apiEndpoint {
	return []apiEndpoint{
		...
		{Method: "POST", Path: "/api/repositories/resolve", ID: "resolveRepository", Summary: "Resolve a registered repository from its root path", Response: domain.RepositorySummary{}, Request: "repositoryPath", Errors: []int{404}, Description: "Read-only exact lookup by an absolute path visible to the engine. Symlinks are canonicalized when available. Returns repository identity and latest snapshot without echoing the path or invoking Git, indexing, or Ollama. In Docker, use the mounted container path rather than a host-only path."},
		{Method: "GET", Path: "/api/workspace-roots", ID: "workspaceRoots", Summary: "List allowed engine filesystem locations", Response: []domain.FolderRoot{}, Description: "Authenticated operator-only server paths. Docker paths must be mounted. This does not browse the caller's filesystem."},
